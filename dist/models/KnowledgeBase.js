...
const aiTrainingDataSchema = new mongoose_1.Schema({
...
const accessSchema = new mongoose_1.Schema({
    ...
    allowedRoles: [String],
    allowedAgents: [{ type: mongoose_1.Schema.Types.ObjectId, ref: 'User' }]
}, { _id: false });
...
const knowledgeBaseSchema = new mongoose_1.Schema({
    ...
    relatedArticles: [{ type: mongoose_1.Schema.Types.ObjectId, ref: 'KnowledgeBase' }],
...
knowledgeBaseSchema.pre('save', async function (next) {
    if (this.isNew) {
        if (!this.articleId) {
            const count = await mongoose_1.default.model('KnowledgeBase').countDocuments();
        ...
        if (!this.seo.slug) {
            ...
            let slug = baseSlug;
            let counter = 1;
            while (await mongoose_1.default.model('KnowledgeBase').findOne({ 'seo.slug': slug })) {
...
// Static methods
knowledgeBaseSchema.statics.searchArticles = function (query, category, language = 'en') {
    const searchQuery = { $text: { $search: query }, status: 'published', language: language, isActive: true };
    if (category) {
...
knowledgeBaseSchema.statics.getPopularArticles = function (limit = 10, category) {
    const matchQuery = { status: 'published', isActive: true };
    if (category)
...
knowledgeBaseSchema.methods.markHelpful = function (isHelpful) {
    if (isHelpful) {
        this.analytics.helpful++;
    }
    else {
        this.analytics.notHelpful++;
    }
    return this.save();
};
knowledgeBaseSchema.methods.addSearchQuery = function (query) {
    ...
    if (!this.analytics.popularQueries.includes(query)) {
        ...
        if (this.analytics.popularQueries.length > 10) {
            this.analytics.popularQueries = this.analytics.popularQueries.slice(-10);
        }
    }
    return this.save();
};
knowledgeBaseSchema.methods.publish = function () {
    ...
    if (this.workflow.reviewRequired && !this.workflow.approvedAt) {
...
knowledgeBaseSchema.methods.addToAITraining = function (intents, entities, sampleQuestions) {
    this.aiTrainingData = { usedForAI: true, intents, entities, sampleQuestions, confidence: 0.8 };
    return this.save();
};
knowledgeBaseSchema.methods.linkRelatedArticles = function (articleIds) {
    this.relatedArticles = [...new Set([...this.relatedArticles, ...articleIds])];
    return this.save();
};
exports.default = mongoose_1.default.model('KnowledgeBase', knowledgeBaseSchema);

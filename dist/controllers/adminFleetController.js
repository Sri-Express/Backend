...
exports.getComplianceIssues = exports.getInspectionsDue = exports.getFleetStats = exports.deleteFleet = exports.reactivateFleet = exports.suspendFleet = exports.rejectFleet = exports.approveFleet = exports.updateFleet = exports.createFleet = exports.getFleetById = exports.getAllFleets = void 0;
...
const getAllFleets = async (req, res) => {
    var _a, _b, _c;
    try {
        const { page = 1, limit = 10, search = '', status = 'all', sortBy = 'applicationDate', sortOrder = 'desc' } = req.query;
        // Build query
        ...
        const fleets = await Fleet_1.default.find(query)
            .populate('approvedBy rejectedBy suspendedBy', 'name email')
            .sort(sort)
            .skip(skip)
            .limit(pageSize);
        // Get total count for pagination
        const totalFleets = await Fleet_1.default.countDocuments(query);
        ...
        const fleetStats = {
            ...
            pendingApprovals: ((_a = stats.find(s => s._id === 'pending')) === null || _a === void 0 ? void 0 : _a.count) || 0,
            approvedFleets: ((_b = stats.find(s => s._id === 'approved')) === null || _b === void 0 ? void 0 : _b.count) || 0,
            rejectedApplications: ((_c = stats.find(s => s._id === 'rejected')) === null || _c === void 0 ? void 0 : _c.count) || 0,
            activeVehicles: stats.reduce((sum, item) => sum + item.activeVehicles, 0),
            complianceIssues: await Fleet_1.default.countDocuments({
...
const getFleetById = async (req, res) => {
    try {
        ...
        const fleet = await Fleet_1.default.findById(id)
...
const createFleet = async (req, res) => {
    try {
        const { companyName, registrationNumber, contactPerson, email, phone, address, totalVehicles, operatingRoutes, documents, financialInfo, operationalInfo, notes } = req.body;
        ...
        const existingFleet = await Fleet_1.default.findOne({ registrationNumber });
        if (existingFleet) {
            res.status(400).json({
        ...
        // Add optional fields
        if (financialInfo)
            fleetData.financialInfo = financialInfo;
        if (operationalInfo)
            fleetData.operationalInfo = operationalInfo;
        if (notes)
            fleetData.notes = notes;
        // Create fleet
        const fleet = await Fleet_1.default.create(fleetData);
        res.status(201).json({
...
const updateFleet = async (req, res) => {
    try {
        ...
        // Update fleet
        const updatedFleet = await Fleet_1.default.findByIdAndUpdate(id, { ...updateData, updatedAt: new Date() }, { new: true, runValidators: true }).populate('approvedBy rejectedBy suspendedBy', 'name email');
        res.json({
...
exports.approveFleet = approveFleet;
...
exports.rejectFleet = rejectFleet;
// @desc    Suspend fleet
...
exports.reactivateFleet = reactivateFleet;
...
const getFleetStats = async (req, res) => {
    ...
    try {
        ...
        const totalApplications = await Fleet_1.default.countDocuments({ isActive: true });
        const pendingApprovals = await Fleet_1.default.countDocuments({ status: 'pending', isActive: true });
        const approvedFleets = await Fleet_1.default.countDocuments({ status: 'approved', isActive: true });
        const rejectedApplications = await Fleet_1.default.countDocuments({ status: 'rejected', isActive: true });
        const suspendedFleets = await Fleet_1.default.countDocuments({ status: 'suspended', isActive: true });
        // Get vehicle statistics
        const vehicleStats = await Fleet_1.default.aggregate([
            { $match: { isActive: true } },
            {
                $group: {
                    _id: null,
        ...
        const complianceIssues = await Fleet_1.default.countDocuments({
            status: { $in: ['approved', 'pending'] },
            complianceScore: { $lt: 70 },
            isActive: true
        });
        // Get average compliance score
        const complianceStats = await Fleet_1.default.aggregate([
            { $match: { isActive: true } },
            {
                $group: {
                    _id: null,
        ...
        const recentApplications = await Fleet_1.default.countDocuments({
            applicationDate: { $gte: thirtyDaysAgo },
            isActive: true
        });
        // Get fleets requiring inspection
        const inspectionDue = await Fleet_1.default.countDocuments({
            status: 'approved',
        ...
        const stats = {
            ...
            suspendedFleets,
            activeVehicles: ((_a = vehicleStats[0]) === null || _a === void 0 ? void 0 : _a.activeVehicles) || 0,
            totalVehicles: ((_b = vehicleStats[0]) === null || _b === void 0 ? void 0 : _b.totalVehicles) || 0,
            complianceIssues,
            recentApplications,
            inspectionDue,
            averageComplianceScore: ((_c = complianceStats[0]) === null || _c === void 0 ? void 0 : _c.avgComplianceScore) || 0,
            complianceRange: {
                min: ((_d = complianceStats[0]) === null || _d === void 0 ? void 0 : _d.minComplianceScore) || 0,
                max: ((_e = complianceStats[0]) === null || _e === void 0 ? void 0 : _e.maxComplianceScore) || 0
            }
        };
        res.json(stats);
    }
...
exports.getInspectionsDue = getInspectionsDue;
...
exports.getComplianceIssues = getComplianceIssues;

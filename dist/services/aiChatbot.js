...
exports.provideFeedback = exports.getResponseSuggestions = exports.analyzeSentiment = exports.getAIResponse = void 0;
...
const getAIResponse = async (message, context = {}, sessionId) => {
    try {
        ...
        const sentiment = (0, exports.analyzeSentiment)(processedMessage);
        ...
        if (sentiment.score < -0.5)
            response.confidence *= 0.8; // Lower confidence for negative sentiment
        if (sentiment.urgency > 0.7)
            response.requiresHuman = true; // Escalate urgent messages
        // Add metadata
        ...
        if (sessionId)
            await updateChatMetrics(sessionId, response);
        return response;
    }
    catch (error) {
...
// Sentiment analysis
const analyzeSentiment = (text, type = 'chat') => {
    ...
    words.forEach(word => {
        ...
        if (SENTIMENT_KEYWORDS.negative.includes(word)) {
            sentimentScore -= 1;
            matchedWords++;
        }
        if (SENTIMENT_KEYWORDS.urgent.includes(word)) {
            urgencyScore += 1;
            matchedWords++;
        }
    });
    ...
    if (normalizedSentiment > 0.1)
        sentiment = 'positive';
    else if (normalizedSentiment < -0.1)
        sentiment = 'negative';
...
const handleGoodbye = async (context) => {
    const goodbyes = [
...
const handleBookingInquiry = async (message, context) => {
    try {
        // Search knowledge base for booking-related articles
        const articles = await KnowledgeBase_1.default.searchArticles(message, 'booking');
...
const handlePaymentIssue = async (message, context) => {
    try {
        // Check if user has recent bookings with payment issues
        if (context.userId) {
            const recentBookings = await Booking_1.default.find({ userId: context.userId, 'paymentInfo.status': { $in: ['pending', 'failed'] } }).limit(3).sort({ createdAt: -1 });
        ...
        const articles = await KnowledgeBase_1.default.searchArticles(message, 'payment');
        if (articles.length > 0) {
            return {
...
const handleTracking = async (message, context) => {
    try {
        if (context.userId) {
            // Get user's recent bookings
            const recentBookings = await Booking_1.default.find({ userId: context.userId, status: { $in: ['confirmed', 'pending'] } }).limit(5).sort({ travelDate: 1 }).populate('routeId', 'name startLocation endLocation');
            if (recentBookings.length > 0) {
...
const handleRouteInfo = async (message, context) => {
    try {
        // Search for route information
        const routes = await Route_1.default.find({ $or: [{ name: { $regex: message, $options: 'i' } }, { 'startLocation.name': { $regex: message, $options: 'i' } }, { 'endLocation.name': { $regex: message, $options: 'i' } }] }).limit(5);
        if (routes.length > 0) {
...
const handleComplaint = async (message, context) => {
    return {
...
const handleTechnicalIssue = async (message, context) => {
    try {
        const articles = await KnowledgeBase_1.default.searchArticles(message, 'technical');
        if (articles.length > 0) {
            return {
...
const handleCancellation = async (message, context) => {
    try {
        if (context.userId) {
            const activeBookings = await Booking_1.default.find({ userId: context.userId, status: 'confirmed', travelDate: { $gte: new Date() } }).limit(5).sort({ travelDate: 1 });
...
const handleGeneral = async (message, context) => {
    try {
        // Search knowledge base for general information
        const articles = await KnowledgeBase_1.default.searchArticles(message);
        if (articles.length > 0) {
...
const getEscalationResponse = (issueType, context) => {
    return {
...
const getResponseSuggestions = async (customerMessage, context = {}) => {
    const intent = classifyIntent(customerMessage.toLowerCase());
    const sentiment = (0, exports.analyzeSentiment)(customerMessage);
    const suggestions = [];
...
exports.getResponseSuggestions = getResponseSuggestions;
// Provide feedback on AI responses for training
const provideFeedback = async (responseId, feedback, rating) => {
    try {
...
const updateChatMetrics = async (sessionId, response) => {
    try {
        const chat = await Chat_1.default.findOne({ sessionId, isActive: true });
        if (chat) {
...
exports.default = { getAIResponse: exports.getAIResponse, analyzeSentiment: exports.analyzeSentiment, getResponseSuggestions: exports.getResponseSuggestions, provideFeedback: exports.provideFeedback };

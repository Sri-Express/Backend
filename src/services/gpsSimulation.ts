...
import LocationTracking from '../models/LocationTracking'; import Route from '../models/Route'; import Device from '../models/Device';

interface RouteCoordinate { lat: number; lng: number; name: string; stop: boolean; }

interface SimulationRoute { name: string; coordinates: RouteCoordinate[]; estimatedDuration: number; distance: number; }

interface VehiclePosition { lat: number; lng: number; accuracy: number; speed: number; heading: number; }

interface SimulationVehicle { vehicleId: string; vehicleNumber: string; routeKey: string; type: string; capacity: number; operator: string; driver: { name: string; id: string; phone: string; }; currentPassengers: number; currentIndex: number; progress: number; speed: number; status: string; departureTime: string; passengers: any[]; lastStop: string | null; nextStop: string | null; currentPosition?: VehiclePosition; lastUpdate?: Date; environmental?: { weather: string; traffic: string; temperature: number; }; delays?: { currentDelay: number; reason?: string; reportedAt?: Date; }; boardingCount?: number; alightingCount?: number; route?: SimulationRoute; }

class AdvancedGPSSimulation {
  private simulatedVehicles = new Map<string, SimulationVehicle>(); private isRunning = false; private intervalId: NodeJS.Timeout | null = null; private speedMultiplier = 1; private routes: Record<string, SimulationRoute>; private vehicles: SimulationVehicle[];

  constructor() { this.routes = this.initializeRoutes(); this.vehicles = this.initializeVehicles(); }

  // REAL SRI LANKAN ROUTES WITH ACCURATE GPS COORDINATES
  private initializeRoutes(): Record<string, SimulationRoute> {
    return {
  ...
  private initializeVehicles(): SimulationVehicle[] {
    return [
  ...
  private interpolatePosition(start: RouteCoordinate, end: RouteCoordinate, progress: number, smoothing = 0.1): { lat: number; lng: number } {
    const smoothProgress = this.easeInOutCubic(progress); const lat = start.lat + (end.lat - start.lat) * smoothProgress; const lng = start.lng + (end.lng - start.lng) * smoothProgress; const jitter = smoothing * 0.001; return { lat: lat + (Math.random() - 0.5) * jitter, lng: lng + (Math.random() - 0.5) * jitter };
  }
  ...
  private calculateHeading(start: RouteCoordinate, end: RouteCoordinate): number { const dLng = (end.lng - start.lng) * Math.PI / 180; const lat1 = start.lat * Math.PI / 180; const lat2 = end.lat * Math.PI / 180; const y = Math.sin(dLng) * Math.cos(lat2); const x = Math.cos(lat1) * Math.sin(lat2) - Math.sin(lat1) * Math.cos(lat2) * Math.cos(dLng); return (Math.atan2(y, x) * 180 / Math.PI + 360) % 360; }

  // REALISTIC SPEED VARIATION BASED ON CONDITIONS
  private getRealisticSpeed(baseSpeed: number, vehicleType: string, isAtStop: boolean, trafficCondition: string, weather: string): number {
    let speed = baseSpeed; if (isAtStop) return 0; if (vehicleType === 'train') speed *= 1.5; else if (vehicleType === 'bus') speed *= (0.8 + Math.random() * 0.4); switch (trafficCondition) { case 'heavy': speed *= 0.4; break; case 'moderate': speed *= 0.7; break; case 'light': speed *= 0.9; break; default: speed *= 1.0; } if (weather === 'rain') speed *= 0.8; else if (weather === 'storm') speed *= 0.6; return Math.max(5, speed + (Math.random() - 0.5) * 10);
  }

  // REALISTIC PASSENGER LOAD SIMULATION
  private simulatePassengerChanges(vehicle: SimulationVehicle, isAtStop: boolean): void {
    if (!isAtStop) return; const maxChange = Math.min(8, Math.floor(vehicle.capacity * 0.15)); const alighting = Math.floor(Math.random() * Math.min(vehicle.currentPassengers, maxChange)); const boarding = Math.floor(Math.random() * Math.min(vehicle.capacity - vehicle.currentPassengers + alighting, maxChange)); vehicle.currentPassengers = Math.max(0, Math.min(vehicle.capacity, vehicle.currentPassengers - alighting + boarding)); vehicle.boardingCount = boarding; vehicle.alightingCount = alighting;
  }

  // WEATHER & TRAFFIC SIMULATION
  private getEnvironmentalConditions() {
    const weather = ['sunny', 'cloudy', 'light_rain', 'rain'][Math.floor(Math.random() * 4)]; const traffic = ['light', 'moderate', 'heavy'][Math.floor(Math.random() * 3)]; const temperature = 24 + Math.random() * 8; return { weather, traffic, temperature };
  }

  // DELAY SIMULATION WITH REALISTIC REASONS
  private simulateDelays(vehicle: SimulationVehicle): void {
    const delayReasons = ['Traffic congestion', 'Mechanical issue', 'Weather conditions', 'Passenger loading', 'Road construction', 'Signal failure']; if (Math.random() < 0.05) { vehicle.delays = { currentDelay: Math.floor(Math.random() * 20) + 5, reason: delayReasons[Math.floor(Math.random() * delayReasons.length)], reportedAt: new Date() }; vehicle.status = vehicle.delays.currentDelay > 15 ? 'delayed' : 'on_route'; } else if (vehicle.delays && Math.random() < 0.3) { vehicle.delays.currentDelay = Math.max(0, vehicle.delays.currentDelay - 1); if (vehicle.delays.currentDelay === 0) { vehicle.status = 'on_route'; vehicle.delays = { currentDelay: 0 }; } }
  }

  // MAIN SIMULATION UPDATE LOOP
  updateVehiclePositions(): void {
    this.vehicles.forEach(vehicle => {
      const route = this.routes[vehicle.routeKey]; if (!route) return; const coordinates = route.coordinates; const currentIndex = vehicle.currentIndex; if (currentIndex >= coordinates.length - 1) { vehicle.currentIndex = 0; vehicle.progress = 0; vehicle.currentPassengers = Math.floor(Math.random() * vehicle.capacity * 0.3); return; }

      const currentStop = coordinates[currentIndex]; const nextStop = coordinates[currentIndex + 1]; const isAtStop = currentStop.stop && vehicle.progress < 0.1; if (isAtStop && Math.random() < 0.3) { vehicle.status = 'at_stop'; this.simulatePassengerChanges(vehicle, true); } else { vehicle.status = vehicle.delays?.currentDelay && vehicle.delays.currentDelay > 5 ? 'delayed' : 'on_route'; }

      const environmental = this.getEnvironmentalConditions(); const currentSpeed = this.getRealisticSpeed(vehicle.speed, vehicle.type, isAtStop, environmental.traffic, environmental.weather); vehicle.progress += (currentSpeed * this.speedMultiplier * 0.001); if (vehicle.progress >= 1) { vehicle.currentIndex++; vehicle.progress = 0; if (currentStop.stop) { this.simulatePassengerChanges(vehicle, true); } }
      ...
      vehicle.lastUpdate = new Date(); vehicle.environmental = environmental; vehicle.lastStop = currentStop.name; vehicle.nextStop = nextStop.name; this.simulateDelays(vehicle);
...
export default new AdvancedGPSSimulation();
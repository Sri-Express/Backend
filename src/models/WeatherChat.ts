...
// Interface for Weather Chat Document
export interface IWeatherChat extends Document {
...
// Interface for Weather Preferences (extends User model)
export interface IWeatherPreferences {
  ...
  alertTypes: ('rain' | 'wind' | 'temperature' | 'visibility' | 'uv')[];
...
// Weather Chat Schema
const WeatherChatSchema: Schema<IWeatherChat> = new Schema({
  userId: {
...
WeatherChatSchema.virtual('ageInDays').get(function(this: IWeatherChat) {
  const now = new Date();
...
WeatherChatSchema.virtual('sessionDuration').get(function(this: IWeatherChat) {
  return this.metadata.responseTime || 0;
});

// Instance Methods
WeatherChatSchema.methods.addFeedback = function(rating: number, comment?: string) {
...
WeatherChatSchema.methods.addTag = function(tag: string) {
...
WeatherChatSchema.statics.getUserStats = function(userId: mongoose.Types.ObjectId) {
  return this.aggregate([
...
WeatherChatSchema.statics.getLocationTrends = function(location: string, days: number = 30) {
...
// Pre-save middleware
WeatherChatSchema.pre('save', function(this: IWeatherChat, next) {
...
WeatherChatSchema.post('save', function(this: IWeatherChat) {
  // Could trigger analytics updates here
  console.log(`💾 Weather chat saved: ${this.location} - ${this.userMessage.slice(0, 50)}...`);
});

// Model creation
const WeatherChat: Model<IWeatherChat> = mongoose.model<IWeatherChat>('WeatherChat', WeatherChatSchema);

export default WeatherChat;

// Export additional types for use in controllers
export type WeatherChatDocument = IWeatherChat;
export type WeatherPreferences = IWeatherPreferences;

// Helper function to validate weather preferences
export const validateWeatherPreferences = (preferences: Partial<IWeatherPreferences>): string[] => {
  const errors: string[] = [];
...
export const getDefaultWeatherPreferences = (): IWeatherPreferences => ({

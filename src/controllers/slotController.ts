// src/controllers/slotController.ts - Route Slot Management Controller (Simplified)
import { Request, Response } from 'express';
import RouteSlot from '../models/RouteSlot';
import SlotAssignment from '../models/SlotAssignment';
...
export class SlotController {
  ...
  static async createRouteSlots(req: AuthRequest, res: Response): Promise<void> {
    try {
  ...
  static async getRouteSlots(req: AuthRequest, res: Response): Promise<void> {
    try {
      const { routeId } = req.params;
      ...
      const slots = await RouteSlot.find({
        routeId: new mongoose.Types.ObjectId(routeId),
        isActive: true
      }).sort({ slotNumber: 1 });

      // Get assignments for each slot
      const slotsWithAssignments = await Promise.all(
  ...
  static async assignVehicleToSlot(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      // Check slot capacity
      const slot = await RouteSlot.findById(slotId).populate('routeId');
      if (!slot) {
        res.status(404).json({ message: 'Slot not found' });
        return;
      }

      const existingAssignments = await SlotAssignment.find({
  ...
  static async updateAssignmentStatus(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const assignment = await SlotAssignment.findById(assignmentId);
      if (!assignment) {
  ...
  static async getPendingAssignments(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ message: 'Authentication required' });
        return;
  ...
  static async getApprovedAssignments(req: AuthRequest, res: Response): Promise<void> {
    try {
      if (!req.user) {
        res.status(401).json({ message: 'Authentication required' });
        return;
      ...
      const assignments = await SlotAssignment.find(query)
        .populate('slotId')
        .populate('vehicleId', 'vehicleNumber vehicleType status')
        .populate('fleetId', 'companyName phone')
  ...
  // Remove assignment
  static async removeAssignment(req: AuthRequest, res: Response): Promise<void> {
    try {
      const { assignmentId } = req.params;

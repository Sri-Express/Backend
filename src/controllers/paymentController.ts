...
export const getPaymentById = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const payment = await Payment.findById(id)
      .populate('bookingId', 'bookingId status travelDate passengerInfo routeId')
      .populate({
        path: 'bookingId',
        populate: {
          path: 'routeId',
...
export const getPaymentHistory = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const payments = await Payment.find(filter)
      .populate('bookingId', 'bookingId travelDate passengerInfo routeId')
      .populate({
        path: 'bookingId',
        populate: {
          path: 'routeId',
  ...
  try {
    const paymentMethods = [
      ...
      },
      {
        ...
        isAvailable: true,
        providers: [
          { id: 'dialog', name: 'Dialog eZ Cash', logo: '/images/dialog.png' },
          { id: 'mobitel', name: 'Mobitel mCash', logo: '/images/mobitel.png' }
        ]
      },
      {
        type: 'bank',
    ...
    res.json({
      paymentMethods: paymentMethods.filter(method => method.isAvailable),
      currency: 'LKR',
      minimumAmount: 50,
      maximumAmount: 50000
    });
...
async function processCardPayment(cardInfo: any, amount: number) {
  ...
  // In real implementation, integrate with payment gateway (Stripe, PayHere, etc.)
  return {
    success: true,
...
async function processMobilePayment(provider: string, amount: number) {
  ...
  return {
    success: true,
    transactionId: `TXN_${provider.toUpperCase()}_${Date.now()}`,
...
async function processBankTransfer(bankInfo: any, amount: number) {
  ...
  return {
    success: true,
    transactionId: `TXN_BANK_${Date.now()}`,

// src/controllers/paymentSimulationController.ts - Simple Payment Gateway Simulation
...
export const simulatePayment = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (booking.paymentInfo.paymentId) {
      payment = await Payment.findById(booking.paymentInfo.paymentId);
    }
...
// @access  Private
export const simulateAllPayments = async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.user) { 
      res.status(401).json({ message: 'Not authorized' }); 
      return; 
    ...
    for (const booking of pendingBookings) {
      try {
        ...
        let payment = await Payment.findById(booking.paymentInfo.paymentId);
...
// @access  Private
export const getPaymentGateway = async (req: Request, res: Response): Promise<void> => {
  try {
    if (!req.user) { 
      res.status(401).json({ message: 'Not authorized' }); 
      return; 

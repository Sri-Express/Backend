// src/controllers/trackingController.ts - FIXED VERSION
...
export const getLiveLocations = async (req: Request, res: Response): Promise<void> => {
  try {
    const { bounds, routeId, vehicleType, status, limit = 50 } = req.query;

    // Build filter query
    const filter: any = { isActive: true, timestamp: { $gte: new Date(Date.now() - 5 * 60 * 1000) } }; // Last 5 minutes
    if (routeId) filter.routeId = routeId;
    if (status) filter['operationalInfo.status'] = status;
    ...
    if (bounds) {
      ...
      } catch (error) { res.status(400).json({ message: 'Invalid bounds format' }); return; }
    }

    // Get latest location for each vehicle
    const vehicles = await LocationTracking.aggregate([
      { $match: filter }, { $sort: { vehicleId: 1, timestamp: -1 } },
      { $group: { _id: '$vehicleId', latestLocation: { $first: '$$ROOT' } } },
      { $replaceRoot: { newRoot: '$latestLocation' } }, { $limit: parseInt(limit as string) },
    ...
    res.json({ vehicles, totalVehicles: vehicles.length, lastUpdate: new Date() });
  } catch (error) {
...
export const getRouteVehicles = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const vehicles = await LocationTracking.getRouteVehicles(route._id as Types.ObjectId);
...
export const updateVehicleLocation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!deviceId || !vehicleId || !location) { res.status(400).json({ message: 'Missing required location data' }); return; }
...
export const getETAForBooking = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const booking = await Booking.findById(bookingId).populate('routeId', 'name startLocation endLocation waypoints');
    ...
    // ✅ FIXED: Cast booking.routeId._id to proper ObjectId type
    const vehicles = await LocationTracking.getRouteVehicles(booking.routeId._id as Types.ObjectId);
    ...
    const relevantVehicle = vehicles.reduce((best, current) => {
      ...
      if (current.operationalInfo.delays.currentDelay < best.operationalInfo.delays.currentDelay) return current;
      return best;
    });
    ...
    // ✅ FIXED: Properly access populated route name
    const routeName = (booking.routeId as any).name || 'Unknown Route';
...
export const getVehicleHistory = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const history = await LocationTracking.find(filter).populate('routeId', 'name startLocation endLocation').sort({ timestamp: -1 }).limit(parseInt(limit as string));
...
export const getTrackingAnalytics = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Build match query
    const matchQuery: any = { timestamp: { $gte: timeRange }, isActive: true };
    if (routeId) matchQuery.routeId = routeId;

    // Get tracking statistics
    const stats = await LocationTracking.aggregate([
      { $match: matchQuery },
      { $group: { _id: null, totalVehicles: { $addToSet: '$vehicleId' }, avgSpeed: { $avg: '$location.speed' }, avgDelay: { $avg: '$operationalInfo.delays.currentDelay' }, avgLoad: { $avg: '$passengerLoad.loadPercentage' }, totalRecords: { $sum: 1 } } },
      { $project: { totalVehicles: { $size: '$totalVehicles' }, avgSpeed: { $round: ['$avgSpeed', 1] }, avgDelay: { $round: ['$avgDelay', 1] }, avgLoad: { $round: ['$avgLoad', 1] }, totalRecords: 1 } }
    ]);
    ...
    const hourlyVolume = await LocationTracking.aggregate([
      { $match: matchQuery },
      { $group: { _id: { hour: { $hour: '$timestamp' }, day: { $dayOfMonth: '$timestamp' } }, count: { $sum: 1 }, uniqueVehicles: { $addToSet: '$vehicleId' } } },
      { $project: { hour: '$_id.hour', day: '$_id.day', count: 1, uniqueVehicles: { $size: '$uniqueVehicles' } } },

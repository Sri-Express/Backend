// src/controllers/trackingController.ts - REAL GPS DATA ONLY VERSION
...
export const getLiveLocations = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const recentRecords = await LocationTracking.countDocuments({
      isActive: true,
...
export const updateVehicleLocation = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const routeId = operationalInfo?.tripInfo?.routeId;
    if (!routeId) {
    ...
    try {
      ...
      await realTimeService.broadcastVehicleUpdate({
        ...
        operationalInfo,
        passengerLoad,
        routeId: route._id?.toString() || routeId,
        timestamp: now
      });
